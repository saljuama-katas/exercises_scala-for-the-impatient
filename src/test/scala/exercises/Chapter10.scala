...
class Chapter10 extends FreeSpec with Matchers {

  "Traits" - {

    "Exercise 1" - {
      """--------
        |The java.awt.Rectangle class has useful methods translate and grow that
        |are unfortunately absent from classes such as java.awt.geom.Ellipse2D.
        |In Scala, you can fix this problem. Define a trait RectangleLike with
        |concrete methods translate and grow. Provide any abstract methods that
        |you need for the implementation, so that you can mix in the trait like this:
        |
    ...
    "Exercise 3" - {
      """--------
        ...
        |Ignore the type parameters (everything inside the [...]). Then give the linearization
        |of the traits.
      """.stripMargin ignore {
    ...
    "Exercise 4" - {
      """--------
        ...
        |The key should be 3 by default, but it should be overridable by the user. Provide
        |usage examples with the default key and a key of –3.
      """.stripMargin ignore {
    ...
    "Exercise 6" - {
      """--------
        ...
        |extends Container, even though it makes no sense to add other components to, say,
        |a JButton. Ideally, the Swing designers would have preferred the design in Figure 10–4.
        |
        |But that’s not possible in Java. Explain why not. How could the design be executed
    ...
    "Exercise 7" - {
      """--------
        ...
        |In Main (also in a separate source file), construct a SavingsAccount and access
        |the new field. Recompile all files except for SavingsAccount and verify that the
        |program works. Now add a field to ConsoleLogger and access it in Main. Again,
        |recompile all files except for SavingsAccount. What happens? Why?
      """.stripMargin ignore {
    ...
    "Exercise 8" - {
      """--------
        ...
        |philosophizing frogs. Reading through contrived hierarchies can be tedious and
        |not very helpful, but designing your own is very illuminating. Make your own silly
        |trait hierarchy example that demonstrates layered traits, concrete and abstract
        |methods, and concrete and abstract fields.
      """.stripMargin ignore {
    ...
    "Exercise 12" - {
      """---------
        |Using javap -c -private, analyze how the call super.log(msg) is translated to Java.
